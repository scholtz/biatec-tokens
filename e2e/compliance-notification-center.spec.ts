...
test.describe('Compliance Notification Center — operator journeys', () => {
  ...
  test('queue stale card shows exact count from mock data', async ({ page }) => {
    ...
    // createDemoEvents() generates events with relative timestamps:
    //   - 6 events are < 5 minutes old   → fresh  → NOT counted
    //   - 1 event  (evt-013) is 8 days old → critical → counted
    // The stale card sums events where freshness is 'stale' OR 'critical'.
    // With relative timestamps this is always 1, regardless of the CI run date.

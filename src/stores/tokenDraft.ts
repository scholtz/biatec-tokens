...
export const useTokenDraftStore = defineStore("tokenDraft", () => {
  ...
  const hasDraft = (): boolean => {
    return currentDraft.value !== null && (currentDraft.value.name !== "" || currentDraft.value.symbol !== "" || currentDraft.value.description !== "");
  };
   ...
   */
  const validateNetworkSwitch = (
    _fromNetwork: NetworkId,
    toNetwork: NetworkId,
  ): {
    compatible: boolean;
    warnings: string[];
    ...
    if (!isDraftCompatibleWithNetwork(toNetwork)) {
      warnings.push(`Your selected token standard (${currentDraft.value.selectedStandard}) is not compatible with ${toNetwork}. You will need to select a different standard.`);
    }

    // Check if compliance metadata needs updating
    if (currentDraft.value.micaMetadata && lastSavedNetwork.value !== toNetwork) {
      warnings.push("Network switch detected. Please review your compliance metadata to ensure it matches the new network requirements.");
    }

    // Check for attestation metadata
    if (currentDraft.value.attestationMetadata) {
      warnings.push("Attestation metadata may need to be updated for the new network.");
    }

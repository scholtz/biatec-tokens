    ...
    <div class="p-6">
      ...
      <div class="flex items-start gap-4 mb-6">
        ...
        <div class="flex-1">
          ...
          <p class="text-gray-300 text-sm">
            {{ requiresReconnection ? "This network switch requires disconnecting your wallet" : "Switch to a different blockchain network" }}
          </p>
        </div>
        <button @click="handleCancel" class="flex-shrink-0 p-2 text-gray-400 hover:text-white rounded-lg hover:bg-white/10 transition-colors" aria-label="Close dialog">
      ...
      <div class="mb-6 grid grid-cols-2 gap-4">
        ...
        <div class="p-4 bg-white/5 rounded-lg border border-white/10">
          ...
          <div v-if="currentNetwork.chainType === 'AVM'" class="text-xs text-gray-500 mt-2">
            {{ currentNetwork.genesisId }}
          </div>
          <div v-else-if="currentNetwork.chainType === 'EVM'" class="text-xs text-gray-500 mt-2">Chain ID: {{ currentNetwork.chainId }}</div>
        </div>

        <!-- Target Network -->
        <div class="p-4 bg-gradient-to-br from-blue-500/10 to-purple-500/10 rounded-lg border-2 border-blue-500/30">
          ...
          <div v-if="targetNetwork.chainType === 'AVM'" class="text-xs text-gray-500 mt-2">
            {{ targetNetwork.genesisId }}
          </div>
          <div v-else-if="targetNetwork.chainType === 'EVM'" class="text-xs text-gray-500 mt-2">Chain ID: {{ targetNetwork.chainId }}</div>
        </div>
      </div>

      <!-- Warnings -->
      <div v-if="warnings.length > 0" class="mb-6 space-y-3">
        <div v-for="(warning, index) in warnings" :key="index" class="p-3 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
      ...
      <div v-if="crossChain" class="mb-6 p-4 bg-purple-500/10 border border-purple-500/30 rounded-lg">
        <div class="flex items-start gap-3">
          <i class="pi pi-info-circle text-purple-400 text-lg"></i>
          <div class="flex-1">
            <h4 class="text-sm font-semibold text-purple-300 mb-2">Cross-Chain Switch Detected</h4>
            <p class="text-xs text-gray-300 mb-3">You're switching from a {{ currentNetwork.chainType }} chain to a {{ targetNetwork.chainType }} chain. This requires:</p>
      ...
      <!-- Testnet warning -->
      <div v-if="targetNetwork.isTestnet" class="mb-6 p-4 bg-orange-500/10 border border-orange-500/30 rounded-lg">
        <div class="flex items-start gap-3">
          <i class="pi pi-shield text-orange-400 text-lg"></i>
          <div class="flex-1">
            <h4 class="text-sm font-semibold text-orange-300 mb-2">Testnet Network</h4>
            <p class="text-xs text-gray-300">This is a test network. Assets and transactions on this network have no real value. Use this network for testing purposes only.</p>
          </div>
        </div>
      </div>
...
import { NetworkInfo } from "../stores/network";

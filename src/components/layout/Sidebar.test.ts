...
const RouterLinkStub = {
  template: '<a :to="to" :aria-current="ariaCurrent" @click="$emit(\'click\')"><slot /></a>',
...
describe('Sidebar Component', () => {
  ...
  describe('WCAG 2.1 AA Accessibility', () => {
    ...
    it('nav has aria-labelledby pointing to section heading (WCAG SC 1.3.1)', () => {
      const wrapper = mount(Sidebar, {
        global: {
          plugins: [router, pinia],
          components: {
      ...
      const nav = wrapper.find('nav');
      expect(nav.attributes('aria-labelledby')).toBe('sidebar-quick-actions-heading');

      // The referenced heading must exist in the DOM
      const heading = wrapper.find('#sidebar-quick-actions-heading');

  ...
  <div class="mica-whitelist-management">
    ...
    <div class="mb-6 flex items-start justify-between">
      ...
      <div class="flex space-x-2">
        <button
          ...
          class="px-4 py-2 bg-blue-500/20 text-blue-400 rounded-lg hover:bg-blue-500/30 transition-colors flex items-center space-x-2"
        >
        ...
        <button
          ...
          :disabled="isExporting"
          class="px-4 py-2 bg-green-500/20 text-green-400 rounded-lg hover:bg-green-500/30 transition-colors flex items-center space-x-2"
        >
    ...
    <Modal :show="showAddModal" @close="closeAddModal">
      ...
      <div class="space-y-4">
        ...
        <Input
          v-model="newAddressReason"
          label="Reason (Required for MICA)"
          placeholder="e.g., Passed KYC/AML verification"
          required
        />

        <Input
          v-model="newAddressRequester"
          label="Requester"
          placeholder="Who requested this whitelist entry?"
        />

        <Input
          v-model="newAddressJurisdiction"
        ...
        <div class="space-y-2">
          <label class="flex items-center space-x-2 text-sm text-gray-300">
            <input
              v-model="newAddressKycVerified"
              type="checkbox"
              class="rounded border-white/20 bg-white/10 text-biatec-accent focus:ring-biatec-accent"
            />
        ...
        <div class="space-y-3">
          ...
          <div class="space-y-2 pl-4">
            <label class="flex items-center space-x-2 text-sm text-gray-300">
              <input
                v-model="newAddressComplianceChecks.sanctionsScreening"
                type="checkbox"
                class="rounded border-white/20 bg-white/10 text-biatec-accent focus:ring-biatec-accent"
              />
            ...
            <label class="flex items-center space-x-2 text-sm text-gray-300">
              <input
                v-model="newAddressComplianceChecks.amlVerification"
                type="checkbox"
                class="rounded border-white/20 bg-white/10 text-biatec-accent focus:ring-biatec-accent"
              />
            ...
            <label class="flex items-center space-x-2 text-sm text-gray-300">
              <input
                v-model="newAddressComplianceChecks.accreditedInvestor"
                type="checkbox"
                class="rounded border-white/20 bg-white/10 text-biatec-accent focus:ring-biatec-accent"
              />
      ...
      <template #footer>
        <div class="flex justify-end space-x-3">
          ...
          <button
            @click="addAddress"
            :disabled="isAddingAddress || !newAddress || !newAddressReason"
      ...
      <div class="space-y-4">
        <div>
          <p class="text-sm text-gray-400 mb-2">
            Upload a CSV file with MICA compliance columns:
          </p>
...
CCCC...DDDD,"AML Check Passed","Jane Smith",true,EU,"Institutional client"
    ...
    <Modal :show="showRemoveModal" @close="showRemoveModal = false">
      ...
      <div class="space-y-4">
        ...
        <div class="space-y-2">
          <label class="block text-sm font-medium text-gray-300">
            Reason for Removal (Required for MICA Audit)
          </label>
          <textarea
        ...
        <p class="text-sm text-gray-400">
          This action will be recorded in the audit trail. The address will need to be re-added if needed.
        </p>
      </div>
...
const filteredEntries = computed(() => {
  ...
  if (searchQuery.value) {
    ...
    filtered = filtered.filter((entry) =>
      ...
      entry.requester?.toLowerCase().includes(query)
    );
  }
...
const exportReport = async (format: 'json' | 'csv') => {
  ...
  try {
    const report = await whitelistService.exportComplianceReport(
      props.tokenId,
      props.network || 'VOI',
      format
    );

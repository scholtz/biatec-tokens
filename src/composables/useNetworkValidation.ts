...
import type { NetworkId, NetworkInfo } from './useWalletManager'
...
export function useNetworkValidation() {
  ...
  const validateCurrentNetwork = (): NetworkMismatchWarning[] => {
    ...
    const currentNetwork = walletManager.currentNetwork.value
    const networkInfo = walletManager.networkInfo.value
    ...
    if (draft?.selectedStandard) {
      const standardWarning = validateNetworkForTokenStandard(
  ...
  const canProceedWithDeployment = computed(() => {
    ...
    // Must be connected
    const isConnected = walletManager.isConnected.value
    
    // Must have validated recently (within 10 seconds)
    const isRecentValidation = lastValidationTime.value 
      ? Date.now() - lastValidationTime.value.getTime() < 10000
      : false
